...
| 2026-03-03 | UX + flow updates: create-from-contract opens Create form (no immediate DB insert), prefill for client/contract/series/number/first line, lock client+contract when prefilled from contract, contract select labels show „nr. X din dd.mm.yyyy”, payment method narrowed to `virament_bancar`/`numerar`, payment-term hydration fix, line unit auto-aligned with contract billing cycle for „Servicii conform contract” lines | — | Includes backward-compat fallbacks for legacy `additional_attributes.billing_cycle` values |
| 2026-03-03 | Refined payment-term UI/UX in Create/Edit Invoice: `Termen de plată` uses preset dropdown (7/14/30/Altă valoare), dynamic two-column layout with inner split only for custom term, custom label `Alt termen (zile)`, and immediate due-date recalculation when switching between preset/custom values | — | Kept logic compatible with existing invoice records during hydration |
| 2026-03-03 | Fixed off-by-one issue for custom payment term days (e.g. 150 showing as 149 after refresh) by switching to UTC calendar-day diff helper in InvoiceResource term/scadență sync logic | — | Prevents DST/timezone drift in day difference calculations |
| 2026-03-03 | Final stabilization for payment-term persistence across repeated save/refresh cycles: removed due_date→term feedback loop, made term hydration derive from record dates only, and normalized calendar-date add/diff paths to avoid progressive decrements (14→13→12) | — | Behavior validated interactively on Edit Invoice flow |
| 2026-03-03 | Hardened invoice numbering integrity: create flow now reserves numbering before insert (prevents null `series/full_number`), edit flow validates duplicate `full_number` with user-facing Romanian error instead of SQL exception, list default sort changed to `number desc`; repaired one malformed legacy row missing `full_number` | — | Prevents recurrence of blank-number rows and duplicate-key crashes during manual renumbering |
