...
- [x] Contract form no longer stores `Tip contract`; `Șablon contract` is used as the contract type.
- [x] Contract form renders `Atribute suplimentare` dynamically from selected `Șablon contract`, instead of hardcoded DDD/Paintball fields.
- [x] Each `Șablon contract` can define its own `custom_fields` and these placeholders are available as `{{attr.<cheie>}}`.
...
| 2026-02-24 | Removed persisted contract `type`: dropped `contracts.type`, removed `ContractType` usage from model/resource/PDF, mapped type semantics to `contract_template_id` (`Șablon contract`), and kept backward-compatible placeholder `{{contract.type}}` resolved from template name. | Enum file cleanup (optional) | Migration `2026_02_24_120000_remove_type_from_contracts_table` applied successfully. |
| 2026-02-24 | Replaced global DDD/Paintball contract fields with per-template dynamic attributes: added `contract_templates.custom_fields`, `contracts.additional_attributes`, migrated legacy `ddd_*` / `paintball_*` data into JSON attributes, dropped legacy columns, and rendered dynamic fields in `ContractResource` based on selected template. Added `{{attr.<cheie>}}` placeholder rendering and template-specific placeholder listing; standard models now prefill both text and default custom fields. | Optional: add validation for unique custom field keys per template at DB level (JSON constraint not trivial). | Migration `2026_02_24_133000_add_dynamic_custom_fields_for_contracts` applied successfully. |
| 2026-02-24 | Removed `title` and `billing_cycle` as first-class Contract properties: deleted form inputs and model fill/cast usage, switched standard contract templates to `{{attr.contract_title}}` and `{{attr.billing_cycle}}`, and adjusted invoice defaults to read billing cycle from `additional_attributes` (fallback Lunar). Updated client contract preview to display `Șablon contract` instead of title. | Optional: data migration from old `title` values into a custom attribute key if historical docs require it. | Migration `2026_02_24_150000_remove_title_and_billing_cycle_from_contracts_table` applied successfully. |
| 2026-02-24 | Added general contract signing date field (`signed_date`) for Romanian format `nr. ... din ...`: updated Contract model/form, added `{{contract.signed_date}}` placeholder, updated standard template headers to `Nr. {{contract.number}} din {{contract.signed_date}}`, and updated invoice line description to use signing date first. | Optional: expose `signed_date` as a visible table column in contracts list if needed. | Migration `2026_02_24_160000_add_signed_date_to_contracts_table` applied successfully (backfilled from `start_date` when missing). |
