...
| 2026-03-03 | UX + flow updates: generate-from-contract now opens Create Proforma prefilled (fără inserare imediată), prefill includes first line „Servicii conform contract...”, lock client+contract on prefilled create form, contract select labels show „nr. X din dd.mm.yyyy”, line unit auto-aligned with contract billing cycle for prefilled service lines | — | Backward-compatible fallback kept for legacy `additional_attributes.billing_cycle` |
